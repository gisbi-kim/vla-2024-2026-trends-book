...
\begin{titlepage}
  ...
  \vspace{2mm}
  {\large Draft v0.4, 2026년 5월\par}
\end{titlepage}

\chapter*{Preface}
이 책은 VLM을 이미 알고 있는 공학계 독자가 VLA(Vision-Language-Action) 모델의 구조와 2024--2026년 연구 흐름을 수업 수준으로 정리할 수 있도록 쓰는 미니북이다. 목표는 모델 이름을 나열하는 것이 아니라, VLA가 실제 로봇 정책이 되기 위해 필요한 행동 표현, 학습 절차, 실시간성, 벤치마크, 안전성의 기술적 조건을 정식화하고 비교하는 것이다. 이 책은 exhaustive survey가 아니라 reading-oriented mini-book이며, 각 장의 key papers는 분야 전체를 대표하는 완전 목록이 아니라 해당 기술 병목을 설명하기 위한 anchor이다. v0.4에서는 References와 annotated catalog를 분리하고, appendix/table layout, emerging-front evidence wording, industrial VLA worked example, assignment difficulty tags를 정리했다.
...
\mainmatter
\part{What is VLA?}
\include{chapters/ch01_why_vla}
\include{chapters/ch02_vla_architecture}
\include{chapters/ch03_reading_framework}
\part{Core Bottlenecks}
\include{chapters/ch04_action_representation}
\include{chapters/ch05_finetuning_posttraining}
\include{chapters/ch06_efficient_vla}
\part{Evaluation in the Physical World}
\include{chapters/ch07_manipulation}
\include{chapters/ch08_benchmarks_datasets}
\include{chapters/ch09_reasoning_vla}
\include{chapters/ch10_world_models_simulation}
\include{chapters/ch11_robustness_safety}
\part{Generalist and Cross-Domain VLA}
\include{chapters/ch12_beyond_manipulation}
\include{chapters/ch13_trend_map_2024_2026}
\part{Research Practice}
\include{chapters/ch14_research_topic_selection}
\include{chapters/ch15_paper_reading_practice}

\appendix
\part{Appendices}
\include{appendices/appA_reading_catalog}
\include{appendices/appB_evidence_ladder}
\include{appendices/appC_glossary}

\backmatter
\chapter*{Bibliographic Note}
이 원고는 2024--2026년 VLA 및 robot foundation model 관련 논문군을 기반으로 구성한 강의용 미니북이다. 상위 폴더의 \texttt{01\_pdfs}, \texttt{02\_summaries}, \texttt{03\_citation\_survey}에는 원문 PDF, 13-question 요약, Google Scholar 인용 조사 결과를 분리해 두었으며, 본문은 이를 수업용 개념 구조와 연구 주제 선정 절차로 재구성한 것이다. 각 장의 key papers는 장별 reading anchor이고, full metadata와 source status는 References와 Appendix의 reading catalog에 정리했다.

\nocite{saycan,rt1,rt2,palme,diffusionpolicy,aloha,openx,libero,octo,openvla,pi0,pi05,openvlaoft,fast,vqvla,smolvla,grootn1,helix,geminirobotics,geminiroboticser,worldvla,vlaos,robotwin2}
