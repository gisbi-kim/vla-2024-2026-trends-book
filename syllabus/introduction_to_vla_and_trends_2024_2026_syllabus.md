...
이 미니북은 2024-2026년 VLA(Vision-Language-Action) 연구 흐름을 입문자가 빠르게 이해하되, 단순 모델 소개가 아니라 실제 로봇 정책으로 가는 기술적 병목까지 읽을 수 있도록 설계한다. 중심 논지는 VLA 연구가 “VLM을 로봇에 붙이는 단계”에서 “행동 표현, post-training, 실시간성, benchmark, robustness를 통해 실제 로봇 정책으로 만드는 단계”로 이동했다는 것이다.
